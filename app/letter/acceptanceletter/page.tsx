...
export default function AcceptanceLetterPage() {
  ...
  useEffect(() => {
    const storedLetters = localStorage.getItem("acceptanceLetters");
    if (storedLetters) {
      ...
      try {
        ...
        console.error("Failed to parse acceptanceLetters:", e);
      }
    }
  }, []);
    ...
    const fetchLetters = async () => {
      try {
        ...
        const filteredData = response.data.data.filter(
          (letter: any) => letter.is_acceptance === 1
        );
        console.log("API response:", response.data);
        setLetters(filteredData); // Store data in letters state
      } catch (error) {
        console.error("Error fetching acceptance letters:", error);
      }
    };
  ...
  useEffect(() => {
    const filtered = letters.filter((letter) => {
      const nim = letter.nomor_induk_mahasiswa
        ...
        : "";
      const program = letter.program_studi_mahasiswa
        ? letter.program_studi_mahasiswa.toLowerCase()
        : "";
      const search = searchQuery.toLowerCase();

      // Filter by nim and program
      const nimMatches = nim.includes(search);
      const programMatches = program.includes(search);

      return nimMatches || programMatches; // Only return letters that match either field
    });
  ...
  // On Acceptance Letter page
  const handleEditLetter = (letterData: Letter) => {
    console.log(letterData);
    router.push(
      `/letter/acceptanceletter/applyLetter?edit=${JSON.stringify(letterData)}`
    );
  };
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="px-4 md:px-20 py-10">
        ...
        <p className="text-gray-500 mb-6">
          Kelola surat penerimaan kerja praktik mahasiswa
        </p>

        <div className="flex flex-wrap gap-3 mb-4">
          <button
            onClick={() => router.push("/letter/acceptanceletter/applyLetter")}
            className="bg-red-600 text-white px-4 py-2 rounded flex items-center gap-2"
          >
            <CirclePlus size={18} /> Apply letter
          </button>
          <button
            onClick={() => router.push("/letter/acceptanceletter/viewTemplate")}
            ...
            <tbody>
              {currentLetters.map((letter, idx) => (
                <tr key={idx} className="border-t">
                  ...
                  <td className="py-2 px-4">{letter.nomor_surat_acceptance}</td>
          ...
          <div className="bg-white rounded-md shadow-lg w-[600px]">
            ...
            <div className="px-6 py-4 space-y-2 text-sm text-black">
              <div className="flex justify-between">
                ...
                <span>{selectedLetter.nomor_surat_acceptance}</span>
              </div>
              <div className="flex justify-between">
            ...
            <div className="px-6 py-4 border-t flex justify-end">
              <button
                onClick={() =>
                  router.push(
                    `/letter/acceptanceletter/showLetter?id=${selectedLetter.id}`
                  )
                }
